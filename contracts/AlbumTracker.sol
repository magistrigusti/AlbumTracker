...
contract AlbumTracker is Ownable {
  ...
  event AlbumStateChanged(
    uint256 _albumIndex, uint256 _stateNum, address _albumAddress
  );
  ...
  function triggerPayment(uint _index) public payable {
    ...
    emit AlbumStateChanged(
      _index, uint256(albums[_index].state), address(albums[_index].album)
    );
  }
  ...
  function triggerDelivery(uint _index) public onlyOwner {
    ...
    emit AlbumStateChanged(_index, uint256(albums[_index].state), address(albums[_index].album));
