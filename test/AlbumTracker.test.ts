...
describe("AlbumTracker", function() {
  ...
  it("should revert delivery f album is not paid for", async function() {
    const { tracker, owner, buyer } = await loadFixture(deploy);
  ...
  it("should deliver album after payment", async function() {
    ...
    await tracker.createAlbum(ethers.parseEther("1"), "test");
    await tracker.connect(buyer).triggerPayment(0, { value: ethers.parseEther("1")});
  ...
  it("should revert if not owner tries to deliver", async function() {
    ...
    await expect(tracker.connect(buyer).triggerDelivery(0))
